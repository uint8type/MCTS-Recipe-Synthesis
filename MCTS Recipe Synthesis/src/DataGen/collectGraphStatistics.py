...
def getInformationDictForGMLFiles(gmlFileZip,desName):
    filePathName = desName+"_"+os.path.splitext(osp.basename(gmlFileZip))[0]+"_step0.bench.graphml"
...
def countGatesAndLongestPathLength():
    ...
    print((os.path.join(GML_LOC,"/.zip")))
    allInfo = Parallel(n_jobs=5)(delayed(getInformationDictForGMLFiles)(zipFile,DESNAME) for zipFile in  tqdm(aigGMLZippedFiles, desc="Processing GraphML Files"))
