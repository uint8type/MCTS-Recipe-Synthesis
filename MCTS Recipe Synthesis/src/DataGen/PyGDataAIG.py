...
def pygDataFromNetworkx(G):
    ...
    data = {}
    nodeCountDict = {"PI": 0,"PO":0,"Internal":0,"DFF":0}
    ...
    if nx.is_directed_acyclic_graph(G):
        ...
        data['longest_path'] = -1  # or compute longest path differently
...
class NetlistGraphDataset(Dataset):
    ...
    def raw_file_names(self):
        ...
        if not os.path.exists(rawFolder):
            ...
            # zippedGraphmlFolders = [os.path.join(self.graphmlLoc,'syn0'+'.zip')]
            # print(zippedGraphmlFolders)
            # origGraphmlFolderZip = os.path.join(os.path.dirname(self.graphmlLoc),'orig',self.des)
            os.mkdir(rawFolder)
            for synZippedFolder in tqdm.tqdm(zippedGraphmlFolders, desc="Unzipping GraphML Files"):
        ...
        print(f"Found {len(graphMLfile)} GraphML files:")  # Debugging
    ...
    def process(self):
        ...
        Parallel(n_jobs=10)(delayed(preprocessGraphData)(gmlFile,self.processed_dir,self.des,synthID2VecDict) for gmlFile in tqdm.tqdm(aigGMLFiles, desc="Processing GraphML Files"))
...
def parseCmdLineArgs():
    ...
    parser.add_argument('--ptdata',required=True, help="Design directory (eg. ~/OPENABC_DATASET/ptdata/designName)")
    parser.add_argument('--designs', required=True, help="Design name")
...
def main():
    ...
    aigDataset = NetlistGraphDataset(cmdArgs.ptdata, cmdArgs.designs, cmdArgs.gs, cmdArgs.synvec)
    # print(sys.argv)
    rawFolder = os.path.join(sys.argv[2], 'raw')
