...
datasetDict =  {
    'set1' : ["new_train.csv","new_test.csv"],
    #'set2' : ["i2c_copy_train.csv","i2c_copy_test.csv"],
...
def plotChart(x,y,xlabel,ylabel,leg_label,title):
    ...
    plt.savefig(osp.join(DUMP_DIR,title+'.png'), bbox_inches='tight')
...
def evaluate_plot(model, device, dataloader):
    ...
    batchData = []
    with torch.no_grad():
        for batch_idx, batch in enumerate(tqdm(dataloader, desc="Iteration",file=sys.stdout)):
            try:
                batch = batch.to(device)
                ...
                if "CUDA error: device-side assert triggered" in str(e):
                    print("This error typically occurs when node type indices are out of bounds")
                    print("Please check that all node types are within the expected range")
                raise
...
def main():
    ...
    # print(DUMP_DIR)
    path = osp.join(ROOT_DIR, 'statistics', 'synthesisStatistics17.pickle')
    print(path)
    ...
    print(f"\nDataset Statistics:")
    print(f"Number of files in training dataset: {len(trainDS)}")
    print(f"Number of files in test dataset: {len(testDS)}")
    print(f"Training dataset root: {trainDS.root}")
    print(f"Training dataset file path: {trainDS.filePath}")

    # print(osp.join(ROOT_DIR,'/statistics/synthesisStatistics.pickle'))
    if IS_STATS_AVAILABLE:
        with open(osp.join(ROOT_DIR, 'statistics', 'synthesisStatistics17.pickle'),'rb') as f:
