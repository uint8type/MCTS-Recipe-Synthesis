# This is SynthNetv3_new folders model.py KIndly change it after using.
import torch
...
allowable_features = {
    ...
    'fanouts': None,  # Continuous feature, no maximum value
    'depth': None     # Continuous feature, no maximum value
}
...
# Node Encoder with Learned Transformation for Continuous Features
class NodeEncoder(torch.nn.Module):
    def __init__(self, emb_dim):
        super(NodeEncoder, self).__init__()

        # Embedding for node type
        ...
        # MLP to transform num_fanouts and depth into embeddings
        self.fanout_mlp = torch.nn.Sequential(
            torch.nn.Linear(1, emb_dim),  # Input is 1-dimensional (num_fanouts)
            torch.nn.ReLU(),
            torch.nn.Linear(emb_dim, 1)
        )
        self.depth_mlp = torch.nn.Sequential(
            torch.nn.Linear(1, emb_dim),  # Input is 1-dimensional (depth)
            torch.nn.ReLU(),
            torch.nn.Linear(emb_dim, 1)
        )
    
    def forward(self, x):
        # x[:, 0]: node type, x[:, 1]: num_inverted_predecessors, x[:, 2]: num_fanouts, x[:, 3]: depth
        node_type_embed = self.node_type_embedding(x[:, 0])

        # Transform num_fanouts and depth into embeddings
        fanouts = x[:, 2].float().reshape(-1, 1)  # Reshape to [batch_size, 1]
        depth = x[:, 3].float().reshape(-1, 1)    # Reshape to [batch_size, 1]

        fanout_embed = self.fanout_mlp(fanouts)
        depth_embed = self.depth_mlp(depth)

        # Concatenate all features
        x_embedding = torch.cat((node_type_embed, x[:, 1].reshape(-1, 1), fanout_embed, depth_embed), dim=1)
        return x_embedding
...
class GNN_node(torch.nn.Module):
    ...
    def forward(self, batched_data):
        ...
        # Concatenate all node features: node_type, num_inverted_predecessors, num_fanouts, depth
        x = torch.cat([
            batched_data.node_type.reshape(-1, 1),
            batched_data.num_inverted_predecessors.reshape(-1, 1),
            batched_data.fanouts.reshape(-1, 1),
            batched_data.depth.reshape(-1, 1)
        ], dim=1)
...
class GNN(torch.nn.Module):
    ...
    def forward(self, batched_data):
        ...
        if batched_data.batch is None:
            # If batch tensor is None, create a single batch
            batch = torch.zeros(h_node.size(0), dtype=torch.long, device=h_node.device)
        else:
            # Ensure batch tensor is on the same device as node features
            batch = batched_data.batch.to(h_node.device)
            
            # Validate batch indices
            if batch.min() < 0:
                raise ValueError("Batch indices cannot be negative")
                
        h_graph1 = self.pool1(h_node, batch)
        h_graph2 = self.pool2(h_node, batch)
